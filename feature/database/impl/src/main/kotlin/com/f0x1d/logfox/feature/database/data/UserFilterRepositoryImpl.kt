...
internal class UserFilterRepositoryImpl @Inject constructor(
    ...
    override fun getAllEnabledAsFlow(): Flow<List<UserFilter>> =
        dao.getAllEnabledAsFlow().map { list -> list.map { it.toDomain() } }

    override suspend fun getAll(): List<UserFilter> =
        dao.getAll().map { it.toDomain() }

    override fun getByIdAsFlow(id: Long): Flow<UserFilter?> =
        dao.getByIdAsFlow(id).map { it?.toDomain() }

    override suspend fun getById(id: Long): UserFilter? =
        dao.getById(id)?.toDomain()

    override suspend fun insert(userFilter: UserFilter) =
        dao.insert(userFilter.toEntity())

    override suspend fun insert(items: List<UserFilter>) =
        dao.insert(items.map { it.toEntity() })

    override suspend fun update(userFilter: UserFilter) =
        dao.update(userFilter.toEntity())

    override suspend fun delete(userFilter: UserFilter) =
        dao.delete(userFilter.toEntity())
