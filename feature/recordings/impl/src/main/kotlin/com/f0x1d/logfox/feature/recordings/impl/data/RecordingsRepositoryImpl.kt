...
internal class RecordingsRepositoryImpl
    @Inject
    constructor(
        ...
        private val recordingsDir =
            File("${context.filesDir.absolutePath}/recordings").apply {
                if (!exists()) mkdirs()
            }

        override suspend fun saveAll(): LogRecording =
            withContext(ioDispatcher) {
                ...
                try {
                    FileOutputStream(recordingFile, true).use { out ->
                        loggingRepository
                            .dumpLogs(terminals.getValue(terminalSettingsRepository.selectedTerminalType))
                            .collect { line ->
                                // It is on IO!
                                val original =
        ...
        override suspend fun createRecordingFrom(lines: List<LogLine>): Unit =
            withContext(ioDispatcher) {
                val recordingTime = System.currentTimeMillis()
        ...
        override suspend fun update(item: LogRecording) =
            withContext(ioDispatcher) {
                logRecordingRepository.update(item)
            }

        override suspend fun delete(item: LogRecording) =
            withContext(ioDispatcher) {
                item.deleteFile()
